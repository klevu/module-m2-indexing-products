...
use Klevu\IndexingProducts\Service\Provider\ProductEntityProvider\Configurable as ConfigurableEntityProviderVirtualType;
...
/**
 * @covers \Klevu\IndexingProducts\Service\Provider\ProductEntityProvider\Configurable::class
 ...
 */
class ConfigurableProductEntityProviderTest extends TestCase
{
    use AttributeTrait;
    ...
    protected function setUp(): void
    {
        ...
        $this->implementationFqcn = ConfigurableEntityProviderVirtualType::class; // @phpstan-ignore-line
    ...
    public function testGet_ReturnsProductData_AtStoreScope(): void
    {
        ...
        $this->assertSame(
            ...
            actual: $productConfigurable1Store1->getName(),
        );
        $this->assertSame(
            expected: Status::STATUS_ENABLED,
            actual: (int)$productConfigurable1Store1->getStatus(),
        ...
        $this->assertCount(expectedCount: 1, haystack: $items2);
        $product1Array = array_filter(
            array: $items2,
            callback: static function (ProductInterface $product) use ($productConfigurable): bool {
                return (int)$product->getId() === (int)$productConfigurable->getId();
        ...
        $productConfigurable1Store2 = array_shift($product1Array);
        $this->assertSame(
            expected: 'Configurable Product 1 Store 2',
            actual: $productConfigurable1Store2->getName(),
        );
        $this->assertSame(
            expected: Status::STATUS_ENABLED,
            actual: (int)$productConfigurable1Store2->getStatus(),
            message: 'Product Status',
        );
        ...
        $this->assertCount(expectedCount: 1, haystack: $items3);
        $product1Array = array_filter(
            array: $items3,
            callback: static function (ProductInterface $product) use ($productConfigurable): bool {
                return (int)$product->getId() === (int)$productConfigurable->getId();
        ...
        $productConfigurable1Store3 = array_shift($product1Array);
        $this->assertSame(
            expected: 'Configurable Product 1 Store 3',
            actual: $productConfigurable1Store3->getName(),
        );
        $this->assertSame(
            expected: Status::STATUS_DISABLED,
            actual: (int)$productConfigurable1Store3->getStatus(),
            message: 'Product Status',
        );
    }
}
